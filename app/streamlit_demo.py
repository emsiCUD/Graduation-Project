...
SOFT_TOKEN_LIMIT = 128             # PhoBERT max_len — surfaces as "truncated" notice
...
analyze = st.button("🔍 Phân tích / Analyze", type="primary", use_container_width=True)
...
if analyze:
    ...
    col3.metric("📏 Max len", f"{SOFT_TOKEN_LIMIT}")

    if out["truncated"]:
        st.info(
            f"✂️ Văn bản dài hơn {SOFT_TOKEN_LIMIT} sub-word tokens — "
